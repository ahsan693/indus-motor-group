...
const [openFaq, setOpenFaq] = useState(null)
	...
	return (
		<div className="min-h-screen bg-black text-zinc-300 iphone:text-[15px]">
			...
			{/* ── Main content ── */}
			<main className="layout-shell finance-stack iphone:pt-6">
				...
				<section className="motion-rise grid items-center gap-6 md:grid-cols-[0.9fr_1.1fr] md:gap-10 iphone:gap-5">
					<div className="space-y-4 iphone:space-y-3">
						...
						<div className="flex flex-wrap items-center gap-3 iphone:gap-2 iphone:pt-1">
							<a href="#finance-application-form" className="ui-btn rounded-full bg-white px-6 py-2.5 text-[16px] font-medium text-black transition-colors hover:bg-zinc-200 iphone:text-[14px] iphone:px-4 iphone:py-2">Apply for Finance</a>
							<Link to="/cars" className="ui-btn rounded-full bg-white px-6 py-2.5 text-[16px] font-medium text-black transition-colors hover:bg-zinc-200 iphone:text-[14px] iphone:px-4 iphone:py-2">Browse Cars &rarr;</Link>
						</div>
					</div>
					...
					<img
						...
						loading="lazy"
						decoding="async"
						className="motion-card h-[300px] w-full rounded-lg border border-zinc-800 object-cover sm:h-[360px] md:h-[430px] iphone:h-[180px]"
					/>
				</section>

				{/* ── Benefits ── */}
				<section className="motion-rise motion-rise-delay-1 iphone:mt-8">
					<div className="grid gap-8 md:grid-cols-[0.9fr_1.1fr] md:items-center md:gap-10">
						...
						<div className="grid gap-3 sm:grid-cols-2 iphone:gap-2">
							{benefits.map((item) => (
								<article key={item.title} className="group motion-card rounded-lg bg-zinc-950 p-5 iphone:p-3">
									<span className="mb-3 inline-flex h-8 w-8 items-center justify-center rounded-md border border-zinc-700 bg-zinc-900 text-zinc-300 iphone:mb-2">
										<img src={ICONS[item.icon]} alt={`${item.title} icon`} className="h-5 w-5 object-contain" />
									</span>
									<h3 className="motion-link-slide text-[16px] font-normal text-white md:text-[18px] iphone:text-[14px]">{item.title}</h3>
									<p className="mt-1.5 text-[16px] leading-6 text-zinc-400 md:text-[18px] iphone:text-[13px] iphone:leading-5 iphone:mt-1">{item.body}</p>
								</article>
							))}
						</div>
					</div>
				...
				{/* ── Process ── */}
				<section className="motion-rise motion-rise-delay-2 iphone:mt-8">
					...
					<div className="mt-12 md:mt-16 grid gap-4 md:grid-cols-3 iphone:mt-5 iphone:gap-4">
						{processSteps.map((item) => (
							<article key={item.title} className="group motion-card space-y-2.5 iphone:space-y-2">
								<div className="overflow-hidden rounded-xl border border-zinc-700">
									<img src={item.image} alt={item.title} loading="lazy" decoding="async" className="motion-media h-56 w-full object-cover iphone:h-[160px]" />
								</div>
								<p className="text-[12px] text-zinc-500 md:text-[14px]">({item.step})</p>
								<h3 className="motion-link-slide text-[16px] font-normal text-white md:text-[18px] iphone:text-[14px]">{item.title}</h3>
								<p className="text-[16px] text-zinc-400 md:text-[18px] iphone:text-[13px] iphone:leading-5">{item.body}</p>
							</article>
						))}
					</div>
				</section>
				...
				<section id="finance-application-form" className="motion-rise scroll-mt-24 space-y-6 md:scroll-mt-28 iphone:mt-8 iphone:space-y-4">
					...
					<div className="grid items-stretch gap-3 lg:grid-cols-[1fr_1.05fr] lg:gap-4 iphone:gap-3">
						<form onSubmit={handleFinanceSubmit} className="space-y-3 rounded-lg border border-zinc-700 bg-black p-3 sm:p-4 md:p-5 iphone:space-y-3 iphone:p-3">
							<div className="space-y-1.5">
								<label className="text-[14px] text-zinc-300 md:text-[16px] iphone:text-[13px]">Vehicle Make</label>
								<input name="vehicleMake" value={formValues.vehicleMake} onChange={handleFormChange} type="text" placeholder="e.g. Volkswagen" className="w-full rounded-md border border-zinc-700 bg-black px-3 py-2.5 text-[16px] text-zinc-100 outline-none placeholder:text-zinc-500 iphone:py-2 iphone:text-[14px]" />
							</div>
							<div className="space-y-1.5">
								<label className="text-[14px] text-zinc-300 md:text-[16px] iphone:text-[13px]">Model</label>
								<input name="model" value={formValues.model} onChange={handleFormChange} type="text" placeholder="e.g. Golf" className="w-full rounded-md border border-zinc-700 bg-black px-3 py-2.5 text-[16px] text-zinc-100 outline-none placeholder:text-zinc-500 iphone:py-2 iphone:text-[14px]" />
							</div>
							<div className="space-y-1.5">
								<label className="text-[14px] text-zinc-300 md:text-[16px] iphone:text-[13px]">Year</label>
								<input name="year" value={formValues.year} onChange={handleFormChange} type="text" placeholder="e.g. 2019" className="w-full rounded-md border border-zinc-700 bg-black px-3 py-2.5 text-[16px] text-zinc-100 outline-none placeholder:text-zinc-500 iphone:py-2 iphone:text-[14px]" />
							</div>
							<div className="space-y-1.5">
								<label className="text-[14px] text-zinc-300 md:text-[16px] iphone:text-[13px]">Monthly Budget</label>
								<select name="monthlyBudget" value={formValues.monthlyBudget} onChange={handleFormChange} className="w-full rounded-md border border-zinc-700 bg-black px-3 py-2.5 text-[16px] text-zinc-100 outline-none iphone:py-2 iphone:text-[14px]">
							...
							<div className="space-y-1.5">
								...
								<input name="salePrice" value={formValues.salePrice} onChange={handleFormChange} type="text" required placeholder="e.g. EUR 18950" className="w-full rounded-md border border-zinc-700 bg-black px-3 py-2.5 text-[16px] text-zinc-100 outline-none placeholder:text-zinc-500 iphone:py-2 iphone:text-[14px]" />
							</div>
							<div className="space-y-1.5">
								...
								<input name="firstName" value={formValues.firstName} onChange={handleFormChange} type="text" required placeholder="e.g. John" className="w-full rounded-md border border-zinc-700 bg-black px-3 py-2.5 text-[16px] text-zinc-100 outline-none placeholder:text-zinc-500 iphone:py-2 iphone:text-[14px]" />
							</div>
							<div className="space-y-1.5">
								<label className="text-[14px] text-zinc-300 md:text-[16px] iphone:text-[13px]">Last Name<span className="text-red-500">*</span></label>
								<input name="lastName" value={formValues.lastName} onChange={handleFormChange} type="text" required placeholder="Murphy" className="w-full rounded-md border border-zinc-700 bg-black px-3 py-2.5 text-[16px] text-zinc-100 outline-none placeholder:text-zinc-500 iphone:py-2 iphone:text-[14px]" />
							</div>
							<div className="space-y-1.5">
								<label className="text-[14px] text-zinc-300 md:text-[16px] iphone:text-[13px]">Email<span className="text-red-500">*</span></label>
								<input name="email" value={formValues.email} onChange={handleFormChange} type="email" required placeholder="e.g. yourname@email.com" className="w-full rounded-md border border-zinc-700 bg-black px-3 py-2.5 text-[16px] text-zinc-100 outline-none placeholder:text-zinc-500 iphone:py-2 iphone:text-[14px]" />
							</div>
							<div className="space-y-1.5">
								<label className="text-[14px] text-zinc-300 md:text-[16px] iphone:text-[13px]">Phone Number</label>
								<input name="phoneNumber" value={formValues.phoneNumber} onChange={handleFormChange} type="tel" placeholder="e.g. +353 87 123 4567" className="w-full rounded-md border border-zinc-700 bg-black px-3 py-2.5 text-[16px] text-zinc-100 outline-none placeholder:text-zinc-500 iphone:py-2 iphone:text-[14px]" />
							</div>

							<button disabled={submitState.submitting} type="submit" className="mt-2 w-full rounded-md bg-zinc-200 py-2.5 text-[16px] font-medium text-black transition-colors hover:bg-white disabled:cursor-not-allowed disabled:opacity-60 iphone:text-[14px] iphone:py-2.5 iphone:mt-1">{submitState.submitting ? 'Submitting...' : 'Apply Now'}</button>
							...
							<div className="space-y-3 pt-1 iphone:space-y-2.5">
								<label className="flex items-start gap-2.5 text-[14px] leading-5 text-zinc-400 md:text-[16px] iphone:text-[13px] iphone:leading-[1.5]">
									<input name="consentShareWithProvider" checked={formValues.consentShareWithProvider} onChange={handleFormChange} required type="checkbox" className="mt-0.5 h-4 w-4 shrink-0 rounded border-zinc-600 bg-black" />
									<span>I agree to my details being shared with a third party finance provider.</span>
								</label>
								<label className="flex items-start gap-2.5 text-[14px] leading-5 text-zinc-400 md:text-[16px] iphone:text-[13px] iphone:leading-[1.5]">
									<input name="consentMarketing" checked={formValues.consentMarketing} onChange={handleFormChange} type="checkbox" className="mt-0.5 h-4 w-4 shrink-0 rounded border-zinc-600 bg-black" />
						...
						<img
							...
							loading="lazy"
							decoding="async"
							className="motion-card h-full min-h-[280px] w-full rounded-lg object-cover min-[390px]:min-h-[340px] sm:min-h-[420px] md:min-h-[520px] iphone:min-h-[160px]"
						/>
					</div>
				</section>

				{/* ── FAQs ── */}
				<section className="motion-rise grid gap-8 lg:grid-cols-[320px_1fr] lg:gap-10 iphone:mt-8 iphone:gap-4">
						...
						{faq.map((item, idx) => {
							...
							return (
								<div key={item.question} className={
									...
									<button
										className="flex w-full items-start justify-between gap-3 py-4 text-left focus:outline-none iphone:py-3"
										onClick={() => setOpenFaq(open ? null : idx)}
										aria-expanded={open}
										aria-controls={`faq-answer-${idx}`}
										type="button"
									>
										<span className="text-[16px] font-normal leading-6 text-zinc-100 md:text-[18px] iphone:text-[14px] iphone:leading-5">{item.question}</span>
										<span className="mt-0.5 text-2xl leading-none text-zinc-300 shrink-0">{open ? '-' : '+'}</span>
									</button>
									{open && (
										<div id={`faq-answer-${idx}`} className="pr-8 pb-4 iphone:pb-3 iphone:pr-4">
											<p className="text-[16px] leading-6 text-zinc-400 md:text-[18px] iphone:text-[13px] iphone:leading-5">{item.details}</p>
										</div>
									)}
								</div>
							)
			...
			{/* ── CTA Banner ── */}
			<section className="relative mx-auto h-[320px] w-full max-w-[1440px] overflow-hidden sm:h-[420px] md:h-[600px] iphone:h-[200px] iphone:mt-8">
				<img
					src={financeCtaImg}
					alt="Finance CTA"
					loading="lazy"
				...
				<div className="hero-content-rise absolute inset-x-0 top-0 hero-shell pt-6 sm:pt-8 md:pt-10 iphone:pt-4">
					...
					<a
						...
						className="ui-btn mt-4 inline-flex rounded-full bg-white px-6 py-2.5 text-[16px] font-medium text-black iphone:text-[14px] iphone:px-4 iphone:py-2 iphone:mt-3"
					>
						Start Application
					</a>
				</div>
			...
			{/* ── Footer ── */}
			<footer className="mt-10 border-b border-zinc-700 bg-black iphone:mt-6">
				<div className="site-footer-shell text-white iphone:text-[14px]">
					<div className="site-footer-grid">
						<div>
							<p className="site-footer-brand">INDUS MOTOR GROUP</p>
							<p className="site-footer-copy text-[13px] leading-tight md:text-[14px] font-normal iphone:text-[13px] iphone:leading-5">
								Quality used cars with transparent pricing, trusted warranty options, and a straightforward buying experience.
							</p>

							<div className="mt-4 flex items-center gap-3 text-white iphone:mt-3">
